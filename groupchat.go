...
type GroupOpenId2ChatIdResponse struct {
	internal.BizResponse
	ChatId string `json:"chat_id"`
}

// GroupOpenId2ChatId 客户群opengid转换
...
func (app workChat) GroupOpenId2ChatId(openid string) (resp GroupOpenId2ChatIdResponse) {
	p := H{"openid": openid}
	queryParams := app.buildBasicTokenQuery(app.getAppAccessToken())
	body, err := internal.HttpPost(fmt.Sprintf("/cgi-bin/externalcontact/groupchat/get?%s", queryParams.Encode()), p)
	if err != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = err.Error()
	} else {
